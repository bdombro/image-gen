...
FRAMES: dict[str, FrameDef] = {
    "idle_front": {
        "normal": f"{BASE} A character standing still, breathing in, hands at sides, front view, directly facing the viewer, face centered.",
    ...
    "idle_down": {
        "normal": f"{BASE} Top-down overhead view of a character, looking down from above, mostly just the top of the head and shoulders visible, character seen from above.",
    ...
    "walk_1": {
        "normal": f"{BASE} A character stepping forward with right leg, arms swinging naturally, side profile facing the right side of the screen.",
    ...
    "walk_2": {
        "normal": f"{BASE} A character in passing pose, feet together, arms at sides naturally, side profile facing the right side of the screen.",
    ...
    "jump": {
        "normal": f"{BASE} A character at apex of jump, legs raised and tucked, arms raised slightly for balance, side profile facing the right side of the screen.",
...
def run_sprite_gen(
    ...
    *,
    ...
    if char_image:
        print(f"📷 Loading character image: {char_image}")
        hub_control: str | Image.Image | None = char_image
    else:
        print(f"📝 Using character prompt: {char_prompt}")
        hub_control = None
    ...
    hub_image: Image.Image | None = None
    paths: list[str] = []
    ...
    for idx, (name, frame) in enumerate(FRAMES.items()):
        ...
        control = hub_image if hub_image is not None else hub_control
        img = engine.generate(
            prompt=frame["normal"],
            seed=seed_base + idx,
            steps=steps,
